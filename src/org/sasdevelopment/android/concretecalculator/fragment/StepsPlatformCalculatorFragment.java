...
public class StepsPlatformCalculatorFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View v = inflater.inflate(R.layout.steps_platform_calculator, container, false);
		...
		mPlatformSpinner = (Spinner)v.findViewById(R.id.platform_spinner);
		mPlatformSpinner.setAdapter(adapter);
		
		mStepsDiagram = (ImageView)v.findViewById(R.id.steps_diagram);
		mPlatformEditText = (EditText)v.findViewById(R.id.platform_edittext);
		...
		mStepsDiagram.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				args.putInt(StepsDiagram.DIAGRAM, STEPS_PLATFORM); 
		...
		mCalculationButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				// Get the PlatformLength data. it is in feet
				if (mPlatformEditText.getText().length() == 0) {
					mPlatformLength = 0.0;
				} else if (mPlatformSpinner.getSelectedItem().toString().equals(getResources().getString(R.string.inches))) {
					mPlatformLength = Double.valueOf(mPlatformEditText.getText().toString());
				} else
					mPlatformLength = (Double.valueOf(mPlatformEditText.getText().toString()) * ONE_FOOT); //mPlatformLength should be in inches
				...
				mTotalRebarLength = (mHasRebarInput) ? mTotalRebarLength = Calculations.calculateWallRebar(mWidth / ONE_FOOT, ((mRun * mNumberOfSteps) + mPlatformLength) / ONE_FOOT, 
							mWidthRebarSpacing, mLengthRebarSpacing) : 0;
				
				int totalSquareFeet = (int)((((mRun * mNumberOfSteps) + mPlatformLength) / ONE_FOOT) * (mWidth / ONE_FOOT)); 
				mCubicYards = Calculations.calculateStepsPlatformCubicYards(mPlatformLength, mRise, mRun, mWidth, mNumberOfSteps);
